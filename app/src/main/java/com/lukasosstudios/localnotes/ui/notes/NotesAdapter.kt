...
class NotesAdapter(
    ...
    inner class NoteViewHolder(private val binding: ItemNoteBinding) :
        ...
        fun bind(note: Note, filter: NoteFilter) {
            ...
            binding.noteCardRoot.setOnLongClickListener { _ ->
                HapticUtils.tick(context)
                onLongPress(note)
                true
            }
            binding.pinAction.setOnClickListener { HapticUtils.tick(context); onTogglePin(note) }
            binding.archiveAction.setOnClickListener { HapticUtils.tick(context); onToggleArchive(note) }
            binding.trashAction.setOnClickListener { HapticUtils.tick(context); onTrash(note) }
            binding.restoreAction.setOnClickListener { HapticUtils.tick(context); onRestore(note) }
            binding.deleteForeverAction.setOnClickListener { HapticUtils.tick(context); onDeleteForever(note) }
        }
    }
